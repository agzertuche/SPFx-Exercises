...
const Indicator2: React.StatelessComponent<IIndicator2Props> = props => {
  ...
  const latestMax = Math.floor(earnedAchievements.length / 3) >= 3 ? Math.floor(earnedAchievements.length / 3) : 3;

  const groupedAchievements = earnedAchievements.sort((a, b) => {
      return b.id - a.id;
    })
    .slice(0, latestMax);

  const trendingCompletedAchievements = groupByProperty(groupedAchievements, "achievementId")
    .slice(0, 3)
    .map(g => {
